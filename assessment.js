...
function addItem(array, item) {
  array.push(item);
  var inArray = array.slice(0, array.length);
  return inArray;
}
...
function removeItem(arr, str) {
	for(var i = 0; i < arr.length; i++) {
		if(arr[i].indexOf()) {
			var newArray = arr.splice(arr[i] === str);
		}
	}
	return newArray;
}
...
function makeChatMessage(message, author) {
  ...
  return this.author + this.message + getDate;
}
...
function coderTest(person) {
  ...
  for(key in person) {
	  if (person[key] === 'Jeremy') {
		  lovesCode = person[key] = 10;
	  }
	  else if (person[key] === 'Brack') {
		  lovesCode = person[key] = 0;
	  }
	  else {
		  lovesCode = person[key] = 5;
	  }
  }
  return lovesCode;
}
